...
<div style="padding: 10px 25px;">
    ...
    <div class="toolbar" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding: 15px 25px; background-color: white; border: 1px solid var(--color-border); border-radius: 10px;">
        <div class="search-box">
            ...
            <input type="text" id="txtSearchLP" class="rounded-input" placeholder="Nhập tên loại phòng hoặc giá..." onkeyup="filterLPTable()" style="width: 250px;">
        ...
        <table class="swing-table" id="tblLP">
            ...
            <tbody>
                <c:forEach var="lp" items="${listLoaiPhong}">
                    <tr onclick="openLPForm('update', this.dataset)" 
                        ...
                        <td class="action-cell">
                                                            <form id="frmDelLP_${lp.getMaLoaiPhong()}" action="loai-phong-data" method="post" style="display:none;">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="maLoaiPhong" value="${lp.getMaLoaiPhong()}">
                                </form>
                                <button type="button" class="btn-swing btn-danger" style="padding: 5px 10px; font-size: 13px;" onclick="event.stopPropagation(); confirmDelete('Bạn có chắc chắn muốn xóa Loại phòng này?', 'loaiphong', this.closest('tr').dataset.id, 'frmDelLP_' + this.closest('tr').dataset.id);">Xóa</button>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>

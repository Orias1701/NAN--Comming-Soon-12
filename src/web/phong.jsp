...
<div style="padding: 10px 25px;">
    ...
    <div class="toolbar" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding: 15px 25px; background-color: white; border: 1px solid var(--color-border); border-radius: 10px;">
        <div class="search-box">
            ...
            <input type="text" id="txtSearchPhong" class="rounded-input" placeholder="Nhập từ khóa..." onkeyup="filterPhongTable()" style="width: 250px;">
        ...
        <table class="swing-table" id="tblPhong">
            ...
            <tbody>
                <c:forEach var="p" items="${listPhong}">
                    <tr onclick="openPhongForm('update', this.dataset)" 
                        ...
                        <td class="action-cell">
                                                            <form id="frmDelPhong_${p.getMaPhong()}" action="phong-data" method="post" style="display:none;">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="maPhong" value="${p.getMaPhong()}">
                                </form>
                                <button type="button" class="btn-swing btn-danger" style="padding: 5px 10px; font-size: 13px;" onclick="event.stopPropagation(); confirmDelete('Bạn có chắc chắn muốn xóa phòng này?', 'phong', this.closest('tr').dataset.id, 'frmDelPhong_' + this.closest('tr').dataset.id);">Xóa</button>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
